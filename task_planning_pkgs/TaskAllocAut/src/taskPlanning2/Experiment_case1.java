...
public class Experiment_case1 {
	...
	public static void main(String args[]) throws ExportException, IOException {
		...
		RegExp regExpr_1 = new RegExp("(afbde)|((ab|ba)((fde)|(def)|(dfe)))|(bd((eaf)|(afe)|(aef)))");
		RegExp regExpr_2 = new RegExp("(afbed)|((ab|ba)((fed)|(edf)|(efd)))|(be((daf)|(afd)|(adf)))");
...
////		System.out.println(robotSwitchConfigs.get(0).autRobotsPair.size());
		...
		Set<Robot> providedRobots1 = new HashSet<>(Arrays.asList(robot3));
		for (int index = 0; index < decompositionSet.size(); index++) {
			...
			robotSetSwitchManned.autRobotsPair.add(
					new RobotConfig(new TaskAutomaton(decompositionSet.get(index).get(1)), providedRobots1)
					);
...
//		System.out.println(robotSwitchConfigs.get(0).autRobotsPair.size());
		...
		for (RobotSetSwitch robotSetConfig : robotSwitchConfigs) {
			...
			for (RobotConfig singleConfig : robotSetConfig.autRobotsPair) {
				System.out.println("---------------------------");
//				System.out.println("Subtask automaton is:" + singleConfig.automaton.AutomatonGraph);
