...
class KepRobot():
    def __init__(self, geometry_name, agent_radius, agent_id,
                 ...
                 time_step=0.05, temp_obstacle=None):

        if temp_obstacle is None:
            temp_obstacle = []
        ...
        self.task_pos_dict = {'A': [0.12, 1.38], 'B': [1.28, 2.76], 'C': [1.28, 0.2], 'D': [2.58, 2.76], \
                              'E': [2.58, 1.38], 'F': [2.58, 0.2], 'G': [3.48, 1.38]
                              # 'A': [0.12, 4.28], 'B': [1.28, 5.66], 'C': [1.28, 3.1], 'D': [2.58, 5.66], \
                              # 'E': [2.58, 4.28], 'F': [2.58, 3.1], 'G': [3.48, 4.28]
                              }
    ...
    def get_taskstate(self, goal):
        ...
        self.cplanner = Robot(self.geometry_name, self.agent_radius, self.agent_id, self.v_max, self.twod_pose, self.time_step)
        ...
        while not result.state:
            ...
            temp_traj = [self.trajectory[step + 1][0],  self.trajectory[step + 1][1] + 2.9, \
                         self.trajectory[step + 1][2]]
            desired_pose = temp_traj
    ...
    def get_odom_pos(self, data):
        ...
        self.actualpos6.y = data.pose.pose.position.y + self.initial_position[1] + 2.90
        # yaw (z-axis rotation)
    ...
    def ctrl1_vel(self, desired_position, desired_velocity):
        ...
        cmd_vel.angular.z = desired_velocity[3] + (
                    K_y * error_array_[1][0] * np.sin(error_array_[2][0]) / error_array_[2][0]) + K_the * \
                            error_array_[2][0]
    ...
    def ctrl2_vel(self, desired_position, desired_velocity):
        ...
        kx = 0.00  # 2.0 for robot 192.168.1.13
        ky = 0.00  # 2.0
        b = 0.1  # 0.1
        ...
        cmd_vel.linear.x = desired_velocity[0] * np.cos(self.actualpos6.theta) + desired_velocity[1] * np.sin(
            self.actualpos6.theta)
        cmd_vel.angular.z = 1.0 / b * (
                    desired_velocity[1] * np.cos(self.actualpos6.theta) - desired_velocity[0] * np.sin(
                self.actualpos6.theta))
        # print 'velocity:' + str(cmd_vel.linear.x) + ',' + str(cmd_vel.angular.z) + ',' + str(theta1 - self.actualpos6.theta)
        return cmd_vel
